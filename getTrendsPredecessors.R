...
### Now count when team is away!!

# Count occurrences. Subset when Phoenix is AWAY
test = subset(extPeaks, extPeaks$team1 == team_name & extPeaks$trendType == "UP")
...
# Statistics statP_UP_away

statP_UP_away <- ddply(test, .(test$pname, test$action, test$points), nrow)
names(statP_UP_away) <- c("Player", "Action", "Points", "Freq")
statP_UP_away = statP_UP_away %>% arrange(desc(Freq))
...
# Explorations
# Count occurrences. Subset when Phoenix is AWAY
test = subset(extPeaks, extPeaks$team1 == team_name & extPeaks$trendType == "DOWN")
...
statP_DOWN_away <- ddply(test, .(test$pname, test$action, test$points), nrow)
names(statP_DOWN_away) <- c("Player", "Action", "Points", "Freq")
statP_DOWN_away = statP_DOWN_away %>% arrange(desc(Freq))

### Combine  home and away datas
