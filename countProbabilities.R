...
# Calculate relative NEU of UP and DOWN
stat_comp_all$freq_neu = stat_comp_all$freq_neu - (stat_comp_all$freq_up + stat_comp_all$freq_down)
...
stat_comp_all = subset(stat_comp_all, freq_neu > 0)

# save result data
saveRDS(stat_comp_all, file = "phoenix_results.rds")

#### LOAD FROM FILE (Phoenix results)
stat_comp_all = readRDS("phoenix_results.rds")
# Some explorations
...
stat_to_plot$type <- factor(stat_to_plot$type, levels=c("UP", "NEUTRAL", "DOWN"))


# Good graph
# Group colors
group.colors <- c(UP = "#009E73", DOWN = "#D55E00", NEUTRAL ="#CCCCCC")
g = ggplot() +
